...
@Component({
  selector: 'app-group-sharepage',
  templateUrl: './group-sharepage.page.html',
  styleUrls: ['./group-sharepage.page.scss'],
})
...
filterItems(ev: any) {
 ...
 async basicProfileClick(ownerUserId){
  let postData;
  postData = {
    userId: ownerUserId,
  }
...
getUserGroups(){
  ...
  let sortOrder = 'true';
  this._pocnService.getUserShareGroups(token, sortOrder)?.subscribe(({ data }) => {
    if(this.loading.isLoading) {
      this.loading.dismiss();
    }
    this.myGroups = data['getUserGroups']['data'];
       this.myGroups.forEach((field, index) => {
        let grpPostImageUrl = environment.grpImgUrl + field.bannerFileName + '.' + field.bannerExtension + '?lastmod=' + Math.random();
       var encoded_url = btoa(grpPostImageUrl).replace(/=/g, "").replace(/\//g, '_').replace(/\+/g, '-');
       ...
       field.grpPostImageUrl = environment.imageProxyUrl + "/" + hmac + path + '?lastmod=' + Math.random();
       });
  });
}
hex2a(hexx:any) {
...
console.log(this.eventUserId)
 ...
 shareGroupPost(){
  this.hideShareGroupbutton = false;
      ...
      (response: SharePostResponse) => {
        if(response.data) {
          if(response.data.sharePost.postStatusResponse.status === "success") {
            this.hideShareGroupbutton = true;
          ...
          this.router.navigateByUrl('/tablinks/post',{ state: {  postMsg: 'sharePost'} } );
            ...
            const spanName = "post-share-group-btn";
            let attributes = {
                userId: this._pocnLocalStorageManager.getData("userId"),
                firstName: this._pocnLocalStorageManager.getData("firstName"),
                lastName: this._pocnLocalStorageManager.getData("lastName"),
            ...
            const eventName = 'post share group';
...
searchGroupData(searchData: any){
  this.searchLoaderStatus = false;
  this._pocnService.getPostGroupSearch(searchData,this._pocnLocalStorageManager.getData("pocnApiAccessToken")).subscribe(({ data }) => {
    this.mySearchGroups = data['getUserGroupsSearch']['data'];
    let searchText = searchData;
    console.log(data['getUserGroupsSearch']['data']);
    if(this.mySearchGroups.length > 0 && searchData != ''){
      this.searchLoaderStatus = true;
      this.showSearchData = false;
      this.hideSearchData = true;
      this.emptyShowResult = true;
    } else if(this.mySearchGroups.length == 0){
      this.searchLoaderStatus = true;
     ...
     this.mySearchGroups.forEach((field, index) => {
      let grpPostImageUrl = environment.grpImgUrl + field.bannerFileName + '.' + field.bannerExtension + '?lastmod=' + Math.random();
     var encoded_url = btoa(grpPostImageUrl).replace(/=/g, "").replace(/\//g, '_').replace(/\+/g, '-');
     var path = "/rs:" + "fit" + ":" + "300" + ":" + "400" + ":" + 0 +
      "/g:" + "no"  + "/" + encoded_url + "." + field.bannerExtension;
