...
export class DialerPage implements OnInit {
  ...
  getDialerCaller() {
    this._pocnService.getDialerCaller(this.token)?.subscribe(({ data }) => {
      this.callerIdNumber =
        data["getDialerCaller"].data[0].name +
        " " +
        data["getDialerCaller"].data[0].phoneNumber.replace(
          this.countryCodeArray,
          ""
        );
  ...
  async showDialerPopOver() {
    ...
    popover.onDidDismiss().then((modalDataResponse) => {
      this.callerIdNumber = modalDataResponse.data
        ? modalDataResponse.data
        : "";
      if (modalDataResponse.data) {
        this.callerIdNumber =
          modalDataResponse.data.name + " " + modalDataResponse.data.phone;
  ...
  writeToInput(value: string, number: string = "callee") {
    if (number === "participant") {
      ...
      localStorage.setItem("dialerNumber", this.dialedPhone);
    }
    return;
  }
