...
export class DialerPage implements OnInit {
  ...
  getDialerCaller() {
    this._pocnService.getDialerCaller(this.token)?.subscribe(({ data }) => {
      const dialerNumber = data['getDialerCaller'].data[0].phoneNumber.replace(new RegExp(`^(${this.countryCodeArray.join('|')})`), '');
      this.callerIdNumber = data['getDialerCaller'].data[0].name + " " + data['getDialerCaller'].data[0].phoneNumber.replace(new RegExp(`^(${this.countryCodeArray.join('|')})`), '');
      console.log(this.callerIdNumber, 'callerIdNumber');
      this.fromNumber = this.countryCodeArray[0] + dialerNumber ;
    })
  }
  ...
  callDialer(){
    // console.log("fromNumber", this.fromNumber,"dialedPhone:",this.dialedPhone, "callerData:",this.callerIdNumber,"callDialerrrrrrrrrrrrrrrrr");
