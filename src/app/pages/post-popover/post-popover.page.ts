...
export class PostPopoverPage implements OnInit {
  ...
  deviceType: string = '';
  showDescriptionErr: boolean = false;
  ...
  constructor(
   ...
   ngOnInit() {
    const spanName = "page-view" + this.router.url.replace(/\//g, '-')+ '-' + "post-popover";
    let attributes = {
...
convertToBase64(file : File) : Observable<string> {
  ...
  async getImageAndroid() {
    ...
    permissionList = await Camera.checkPermissions();
    console.log("%cchecking permissions.....", permissionList, 'color: yellow')
  ...
  async presentAlert() {
    let alert = await this.alertController.create({
      ...
      message: 'You must allow the app for using camera/gallery',
      buttons: ['Cancel', {
        text: 'Go to Settings',
        handler: () => {
  ...
  async cameraImageUpload(image: string) {
        ...
        (response: CreatePostResponse) => {
          if(response.data) {
            ...
            if(response.data.createPost.postStatusResponse.status === "success") {
              ...
              const spanName = "post-create-btn";
              let attributes = {
                  userId: this._pocnLocalStorageManager.getData("userId"),
                  firstName: this._pocnLocalStorageManager.getData("firstName"),
                  lastName: this._pocnLocalStorageManager.getData("lastName"),
              ...
              const eventName = 'post create';
              const event =  { 'userEmail': this._pocnLocalStorageManager.getData("userEmail"), 'status': 'success', 'message': 'successfully created post' }
              this.telemetry.sendTrace(spanName, attributes, eventName, event).then((result: string) => {
                  this.telemetry.parentTrace = result;
              })
            }
  ...
  async close(data,f:NgForm) {
    if(f.value['postDescription'] || this.postFileName){
     ...
     // this.closeAlert();
      ...
      //   component: PostAlertPopoverPage,
      //   cssClass: 'alert-post-modal',
      //   // componentProps: {
      //   //   'memberId': postData,
      //   //   'type': 'pocnUser'
      //   // }
