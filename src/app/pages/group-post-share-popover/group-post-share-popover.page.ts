...
@Component({
  selector: 'app-group-post-share-popover',
  templateUrl: './group-post-share-popover.page.html',
  styleUrls: ['./group-post-share-popover.page.scss'],
})
export class GroupPostSharePopoverPage implements OnInit {
  ...
  ngOnInit() {
    const spanName = "page-view" + this.router.url.replace(/\//g, '-') + '-'+ "group-post-share-popover";
    let attributes = {
...
submitPostForm(f:NgForm){
  ...
  if(this.idfileErrorStatus == false &&  this.postImageErrorMsg == false){
    ...
    this.postData = {
      ...
      // postContent: f.value['postDescription'] ,
      // postTags: "",
      // postTitle: "",
      ...
      postType: 0,
      postTypeContent:  this.attachmentTypeContent ,
      postContent: btoa(unescape(encodeURIComponent(this.currentUrl + '==*' +  "==>" + f.value['postDescription']  + "==>" ))),//this.currentUrl + '==*' +  "==>" + f.value['postDescription']  + "==>",
      postTags: "",
      postTitle: "",
      device:this.deviceType,
      parentPost:'',
    ...
    this._pocnService.createPost(this.postData).subscribe(
      (response: CreatePostResponse) => {
        if(response.data) {
          ...
          console.log(attributes);
          const eventName = 'share post group';
          const event =  { 'userEmail': this._pocnLocalStorageManager.getData("userEmail"), 'status': 'failed', 'message': 'failed to share the post from grp' }
          this.telemetry.sendTrace(spanName, attributes, eventName, event).then((result: string) => {
              this.telemetry.parentTrace = result;
          })
          }
  ...
  // else{
  //   if(this.currentUrls == '' || !this.currentUrls || this.currentUrls == null){
  //     this.disablePostBtn = false;
