...
@Component({
  selector: 'app-quote-popover',
  templateUrl: './quote-popover.page.html',
  styleUrls: ['./quote-popover.page.scss'],
})
export class QuotePopoverPage implements OnInit {
  ...
  ngOnInit() {
    const spanName = "page-view" + this.router.url.replace(/\//g, '-') + '-'+ "quote-popover";
    let attributes = {
  ...
  async getImageAndroid() {
    ...
    permissionList = await Camera.checkPermissions();
    console.log("%c checking permissions.....", 'color: yellow')
      ...
      (response: CreatePostResponse) => {
        if(response.data) {
          if(response.data.createPost.postStatusResponse.status === "success") {
            ...
            const spanName = "comment-post-create-btn";
            let attributes = {
                userId: this._pocnLocalStorageManager.getData("userId"),
                firstName: this._pocnLocalStorageManager.getData("firstName"),
                lastName: this._pocnLocalStorageManager.getData("lastName"),
            ...
            const eventName = 'comment post create';
